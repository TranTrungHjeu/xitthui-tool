    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          console.error("Refresh token backend response indicated failure:", response.data.error);
          processQueue(new Error(response.data.error || "Refresh token failed"), null);
          logout();
          return Promise.reject(new Error(response.data.error || "Refresh token failed"));
        }
      } catch (refreshError) {
