...
export function TeacherScheduleModal({
  ...
  const getScheduleStyle = (type: string) => {
    switch (type) {
      ...
      case "OFFICE_HOURS":
        return "bg-warning/80 text-foreground border-warning";
      case "AVAILABLE":
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[95vw] md:max-w-6xl lg:max-w-7xl p-0 gap-0 overflow-hidden bg-muted/50 flex flex-col h-[90vh] md:h-[85vh]">
        {/* Header Block */}
        <DialogHeader className="px-6 py-3 border-b bg-card shrink-0 shadow-sm z-10">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
            ...
            <div className="flex gap-3 text-[11px] font-medium text-foreground items-center">
              <div className="flex items-center gap-1">
                <div className="w-2.5 h-2.5 rounded-sm bg-warning border border-warning"></div>
                Lớp học
              </div>
              <div className="flex items-center gap-1">
                <div className="w-2.5 h-2.5 rounded-sm bg-warning/80 border border-warning"></div>
                Office Hours
              </div>
              <div className="flex items-center gap-1">
                <div className="w-2.5 h-2.5 rounded-sm bg-success/15 border border-success/30"></div>
                Available
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden flex flex-col min-w-0">
          <div className="min-w-[850px] flex-1 flex flex-col bg-card">
            {/* Header row with Days */}
            <div className="flex border-b bg-muted/50/80 shrink-0 select-none">
            ...
            <div className="flex-1 flex relative min-h-0">
              {/* Time column */}
              <div className="w-16 shrink-0 border-r border-border bg-muted/50/30 flex flex-col justify-between py-1 relative select-none">
                {HOURS.map((hour, i) => (
                  <div
                    key={hour}
                    className="relative text-[10px] text-muted-foreground text-right pr-2 flex-1 flex items-start justify-end"
                  >
                ...
                <div className="relative text-[10px] text-muted-foreground text-right pr-2 h-0 flex items-start justify-end">
              ...
              <div className="flex-1 relative min-h-0 h-full">
                ...
                <div className="absolute inset-0 pointer-events-none flex flex-col justify-between">
                  {Array.from({ length: TOTAL_HOURS + 1 }).map((_, i) => (
                    <div
                      key={i}
                      className="border-t border-border/60 w-full flex-1 first:border-t-0"
                    />
                  ))}
                </div>
