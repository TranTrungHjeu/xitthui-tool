...
const slotsCache = new Map<string, SubstituteSlot[]>();
...
export const SubstituteTab: React.FC<SubstituteTabProps> = ({
  selectedDate,
  userCentres,
  onError,
  performedBy,
  ...
  const [slots, setSlots] = useState<SubstituteSlot[]>([]);
  ...
  const loadSlots = async (force = false) => {
    ...
    onError(null);
    try {
      const res = await spreadsheetService.getSubstituteSlots(selectedDate, userCentres);
      if (res.success) {
        ...
        throw new Error(res.error || "Không thể tải danh sách slot dạy thay.");
      }
    } catch (err: any) {
  ...
  const ensureTeachersLoaded = async (slot: SubstituteSlot) => {
  ...
  const handleOpenDropdown = (slot: SubstituteSlot) => {
  ...
  const handleAssign = async (slot: SubstituteSlot, role: BookingRole, teacher: BookableTeacher) => {
    setSavingSlotId(slot.slotId);
    try {
      const res = await spreadsheetService.assignBookTeacher({
        bookingType: BOOKING_TYPES.SUBSTITUTE,
  ...
  const handleUnassign = async (slot: SubstituteSlot, role: BookingRole) => {
    setSavingSlotId(slot.slotId);
    try {
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-muted/20">
      <div className="p-1.5 bg-card border-b border-border flex items-center justify-between shrink-0">
        <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
          <UserCog className="inline h-3 w-3 mr-1" /> Book dạy thay
        </span>
        <Button
      ...
      <div className="flex-1 overflow-auto p-2.5 relative custom-scrollbar">
        {showLoading ? (
          ...
          <div className="h-64 flex flex-col items-center justify-center gap-2 text-muted-foreground bg-card rounded-xl border border-border/60 p-6 shadow-sm">
            <UserCog className="h-8 w-8 text-muted-foreground opacity-50" />
            <p className="text-sm font-medium text-foreground">
              Không có ca nào cần dạy thay trong ngày này
            </p>
            <p className="text-xs text-muted-foreground">
              Mọi buổi học đều đã có giáo viên chính hoặc đã có phân công dạy thay.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full border-collapse text-xs text-left">
              <thead className="bg-muted border-b border-border sticky top-0 z-10">
                <tr>
                  ...
                  <th className="py-2 px-3 font-bold text-foreground w-[25%] text-[11px]">Lớp / Buổi</th>
                  <th className="py-2 px-3 font-bold text-foreground w-[18%] text-[11px]">Môn</th>
                  <th className="py-2 px-3 font-bold text-foreground w-[22%] text-[11px]">GV hiện tại</th>
                  <th className="py-2 px-3 font-bold text-foreground w-[20%] text-[11px]">Dạy thay</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border bg-card">
                {slots.map((slot) => {
                  ...
                  const availableRoles: BookingRole[] = slot.availableRoles || [];
                  const currentRoles = slot.currentTeachers.map((t) =>
                    (t.role || "").toUpperCase(),
                  );
                  const hasLec = currentRoles.includes("LEC") || currentRoles.includes("LECTURER");
                  const hasTa = currentRoles.includes("TA") || currentRoles.includes("TEACHING_ASSISTANT");
                    ...
                    <tr key={slot.slotId} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="py-2 px-3 align-top">
                        ...
                        <div className="text-[10px] text-muted-foreground mt-0.5">
                          {slot.sessionIndex ? `Buổi ${slot.sessionIndex}` : ""}
                      ...
                      <td className="py-2 px-3 align-top">
                        {slot.currentTeachers.length === 0 ? (
                          <span className="text-xs text-muted-foreground italic">Chưa có</span>
                        ) : (
                          <div className="space-y-0.5">
                            {slot.currentTeachers.map((t) => (
                              <div key={t.id} className="text-xs flex items-center gap-1.5">
                                <span
                                  className={`px-1 rounded text-[9px] font-bold ${
                                    (t.role || "").toUpperCase().includes("LEC")
                                      ? "bg-[#E31F26] text-white"
                                      : "bg-[#FFD62D] text-slate-950"
                                  }`}
                                >
                                  {(t.role || "").toUpperCase().includes("LEC") ? "GV" : "TG"}
                                </span>
                                <span className="text-foreground font-medium">{t.fullName}</span>
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      <td className={`py-2 px-3 align-top ${open ? "relative z-30" : ""}`}>
                        {saving ? (
                          ...
                          <span className="text-[10px] text-muted-foreground italic">
                            {hasLec && hasTa ? "Đã đủ GV/TG" : "Không có role khả dụng"}
                          </span>
                        ) : (
                                ...
                                <div className="flex items-center gap-1.5">
                                  <span
                                    className={`px-1.5 py-0.5 rounded text-[9px] font-bold shrink-0 ${
                                      role === BOOKING_ROLES.LEC
                                        ? "bg-[#E31F26] text-white"
                                        : "bg-[#FFD62D] text-slate-950"
                                    }`}
                                  >
                                    {role === BOOKING_ROLES.LEC ? "GV" : "TG"}
                                  </span>
                                  <button
                                    onClick={() => handleOpenDropdown(slot)}
                                    disabled={role !== availableRoles[0]}
                                    className={`flex-1 text-left text-[11px] border border-border rounded-md px-2 py-1 ${
                                      role === availableRoles[0]
                                        ? "bg-card hover:bg-muted/60 cursor-pointer"
                                        : "bg-muted/40 cursor-not-allowed text-muted-foreground"
                                    } font-semibold transition-colors flex items-center justify-between`}
                                  >
                                    <span>-- Chọn GV dạy thay --</span>
                                ...
                                {role === availableRoles[0] && open && (
                                  <>
                                    ...
                                    <TeacherDropdown
                                      ...
                                      onSelect={(t) => handleAssign(slot, role, t)}
