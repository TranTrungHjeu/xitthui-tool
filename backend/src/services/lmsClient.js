...
class LMSClient {
  ...
  async getTeacherSchedules(teacherId, dateGte, dateLte) {
    console.log(`[LMSClient] getTeacherSchedules start for teacher: ${teacherId}`);
    try {
      const query = `
      ...
      if (res.data.errors) {
        console.error(`[LMSClient] findTeacherSchedule errors for ${teacherId}:`, res.data.errors);
        throw new Error(res.data.errors[0].message);
      }

      return res.data.data?.findTeacherSchedule?.data || [];
    } catch (err) {
      console.error(`[LMSClient] getTeacherSchedules failed for ${teacherId}:`, err.message);
      throw err;
    }
  }

  async getTeachers(centers = ["6443460f94300678908f7974"], pageIndex = 0, itemsPerPage = 100) {
